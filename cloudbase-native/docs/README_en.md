# Docs Index

Every document in this directory is provided in two versions:

- Chinese: `*_cn.md`
- English: `*_en.md`

## Start Here

1. `overview/system-reference_en.md`
2. `overview/tech-stack_en.md`
3. `overview/technical-challenges_en.md`
4. `maintenance/maintainer-guide_en.md`

## Overview

- `overview/system-reference_cn.md`
- `overview/system-reference_en.md`
- `overview/tech-stack_cn.md`
- `overview/tech-stack_en.md`
- `overview/technical-challenges_cn.md`
- `overview/technical-challenges_en.md`

## Architecture

- `architecture/architecture_cn.md`
- `architecture/architecture_en.md`
- `architecture/cloud-functions_cn.md`
- `architecture/cloud-functions_en.md`
- `architecture/data-model_cn.md`
- `architecture/data-model_en.md`
- `architecture/frontend-shell_cn.md`
- `architecture/frontend-shell_en.md`
- `architecture/task-state-machine_cn.md`
- `architecture/task-state-machine_en.md`

## Product

- `product/product-scope_cn.md`
- `product/product-scope_en.md`
- `product/glossary-and-rules_cn.md`
- `product/glossary-and-rules_en.md`
- `product/roadmap_cn.md`
- `product/roadmap_en.md`

## Maintenance

- `maintenance/maintainer-guide_cn.md`
- `maintenance/maintainer-guide_en.md`
- `maintenance/change-playbook_cn.md`
- `maintenance/change-playbook_en.md`
- `maintenance/config-matrix_cn.md`
- `maintenance/config-matrix_en.md`
- `maintenance/development-workflows_cn.md`
- `maintenance/development-workflows_en.md`
- `maintenance/integration-checklist_cn.md`
- `maintenance/integration-checklist_en.md`
- `maintenance/operations_cn.md`
- `maintenance/operations_en.md`
- `maintenance/release-and-rollback_cn.md`
- `maintenance/release-and-rollback_en.md`
- `maintenance/troubleshooting_cn.md`
- `maintenance/troubleshooting_en.md`

Important for public checkouts:

- `maintenance/development-workflows_en.md` explains how private runtime files
  are restored from a local zip without changing the normal development flow.

## Reference

- `reference/sample-data-reference_cn.md`
- `reference/sample-data-reference_en.md`


