...
function getTemplateConfig() {
  return Object.keys(TEMPLATE_ENV_MAP).reduce((result, scene) => {
    const templateId = String(process.env[TEMPLATE_ENV_MAP[scene]] || '').trim()
    result[scene] = templateId
    return result
  }, {})
}
...
function applySubscribeResult(current, resultMap, templates, currentTime) {
  const next = normalizePreferences(current, templates)
  const payload = resultMap && typeof resultMap === 'object' ? resultMap : {}
  const nowIso = (currentTime instanceof Date ? currentTime : new Date()).toISOString()
  Object.keys(TEMPLATE_ENV_MAP).forEach((scene) => {
    const templateId = templates[scene]
    if (!templateId) return
    const status = payload[templateId]
    if (!status) return
    if (status === 'accept') {
      next[scene] = {
        templateId,
        ...
        updatedAt: nowIso,
        authorizedAt: nowIso,
        lastSentAt: next[scene] && next[scene].lastSentAt ? next[scene].lastSentAt : '',
      }
      return
    }
    if (status === 'reject') {
      next[scene] = {
        ...
        status: 'rejected',
        updatedAt: nowIso,
        authorizedAt: next[scene] && next[scene].authorizedAt ? next[scene].authorizedAt : '',
        lastSentAt: next[scene] && next[scene].lastSentAt ? next[scene].lastSentAt : '',
      }
      return
    }
...
function normalizeValueByKey(key, value) {
  ...
  if (lower.startsWith('phrase')) {
    return normalizePhraseValue(raw)
  }
  if (lower.startsWith('time') || lower.startsWith('date')) {
    return formatWxTime(raw) || raw
  }
